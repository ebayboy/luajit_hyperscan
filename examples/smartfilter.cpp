...
static int on_match(unsigned int id, unsigned long long from,
	...
	filter_context_t *fctx = (filter_context_t *)ctx;
...
void *filter_new(const char *name, const char **patterns, unsigned int *flags, unsigned int *ids, size_t size)
{
	...
	strncpy(filter->name, name, sizeof(name));
	for (i = 0; i < size; i++) {
	...
	//build database
	if (hs_compile_multi(filter->patterns.data(), 
				filter->flags.data(),
				filter->ids.data(),
		...
		fprintf(stderr, "ERROR: Unable to compile pattern \"%s\": %s\n", filter->patterns, compile_err->message);
		goto error;
	}
...
int filter_match(void *filter, const char *inputData, size_t dlen)
{
	...
	filter_context_t ctx = { f, inputData, dlen };
